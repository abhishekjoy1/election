...
def seat_count(seat_id, state_id):#the task to run for a district under a state
    ...
    if not flag: #if success
        ...
        flag = os.system(cmd) #system call
        if not flag:
...
def state_count(state_id):
    file_name = 'voting_data/Country/state'+str(state_id)+".txt" #if output already exists then hadoop will not run
    if os.path.exists(file_name):
        return True
    seat_ids = State.objects.get(pk=state_id).seat_set.all().values('id') #State.objects.get(pk=state_id) gives the state object. seat_set.all().values('id') gives the ids
    #of the districts under that state
    for seat_id in seat_ids:
        seat_count(seat_id['id'], state_id) #to compute the result for each district we are calling the previous method
    hadoop_input_dir = 'voting_data/State'+str(state_id) #hadoop input for each state that contains the casted votes
    if not os.path.exists(hadoop_input_dir):#if input directory does not exist then there is nothing to run on hadoop
        return False
    hadoop_output_dir = 'Result_State'+str(state_id)#output folder that is generated for each state
    cmd = "hadoop fs -put "+ "voting_data/State"+str(state_id)+ " /user/joy/" #command to copy the input of hadoop from local file system to hdfs
    flag = os.system(cmd) #flag contains the status of the command-nonzero means failure otherwise successs
    if not flag: #if success
        cmd = "hadoop jar VoteCount.jar "+ "/user/joy/State"+str(state_id) + " " + "/user/joy/"+hadoop_output_dir #command to run the job on hadoop for each state
        flag = os.system(cmd)
        if not flag:
            cmd = "hadoop fs -ls /user/joy/"+hadoop_output_dir+'/part-r-00000' #command to check whether output has been generated by hadoop
            flag = os.system(cmd)
            ...
            file_name = 'voting_data/Country/state'+str(state_id)+".txt"
            f = open(file_name, 'a+' )
            f.write(winner+" 1\n")
            f.close()
            return True
...
@task()
def country_count():
    file_name = 'voting_data/country.txt' #if output already exists then hadoop will not run
    if os.path.exists(file_name):
        return True
    state_ids = State.objects.all().values('id') #gives all the state ids
    for state_id in state_ids:
        state_count(state_id['id']) #to compute the result for each district we are calling the previous method
    hadoop_input_dir = 'voting_data/Country' #hadoop input for each state that contains the casted votes
    if not os.path.exists(hadoop_input_dir):#if input directory does not exist then there is nothing to run on hadoop
        return False
    hadoop_output_dir = 'Result_Country'#output folder that is generated for country
    cmd = "hadoop fs -put "+ "voting_data/Country"+ " /user/joy/" #command to copy the input of hadoop from local file system to hdfs
    flag = os.system(cmd) #flag contains the status of the command-nonzero means failure otherwise successs
    if not flag: #if success
        cmd = "hadoop jar VoteCount.jar "+ "/user/joy/Country" + " " + "/user/joy/"+hadoop_output_dir #command to run the job on hadoop for country
        flag = os.system(cmd)
        if not flag:
            cmd = "hadoop fs -ls /user/joy/"+hadoop_output_dir+'/part-r-00000' #command to check whether output has been generated by hadoop
            flag = os.system(cmd)
            ...
            file_name = 'voting_data/country.txt'
            f = open(file_name, 'a+' )
            f.write(winner+" 1\n")
            f.close()
            return True
